...
public abstract class StmtGraph<V extends BasicBlock<V>> implements Iterable<Stmt> {
     ...
     */
    @Nonnull
    public abstract Stream<Stmt> getNodes();
     ...
     */
    @Nonnull
    public abstract Stream<Stmt> predecessors(@Nonnull Stmt node);

    /**
     * it is possible to reach traphandlers through inline code i.e. without any exceptional flow
     */
    @Nonnull
    public abstract Stream<Stmt> exceptionalPredecessors(@Nonnull Stmt node);

    /**
     * returns the outgoing flows of node as ordered List. The List can have duplicate entries!
     */
    @Nonnull
    public abstract Stream<Stmt> successors(@Nonnull Stmt node);
     ...
     */
    @Nonnull
    public Stream<Stmt> getAllSuccessors(@Nonnull Stmt stmt) {
        final Stream<Stmt> successors = successors(stmt);
        final Map<ClassType, Stmt> exSuccessors = exceptionalSuccessors(stmt);
        return Stream.concat(successors, exSuccessors.values().stream());
    }
    ...
    public void validateStmtConnectionsInGraph() {
        try {
            ...
            getNodes().forEach(stmt -> {
                final List<Stmt> successors = successors(stmt).collect(Collectors.toList());
                final int successorCount = successors.size();

                if (!predecessors(stmt).findAny().isPresent()) {
    ...
    public List<Stmt> getExtendedBasicBlockPathBetween(@Nonnull Stmt from, @Nonnull Stmt to) {
        ...
        while (pathStackIndex.get(0) != psiMax) {
            ...
            List<Stmt> succs = successors((pathStack.get(level))).collect(Collectors.toList());
            if (p >= succs.size()) {
    ...
    public boolean equals(Object o) {
        ...
        Collection<Stmt> nodes = getNodes().collect(Collectors.toList());
        final Collection<Stmt> otherNodes = otherGraph.getNodes().collect(Collectors.toList());
        ...
        for (Stmt node : nodes) {
            ...
            // TODO: compare Streams directly
            final List<Stmt> successors = successors(node).collect(Collectors.toList());
            final List<Stmt> otherSuccessors = otherGraph.successors(node).collect(Collectors.toList());
    ...
    public Stream<Stmt> getBranchTargetsOf(BranchingStmt fromStmt) {
        final Stream<Stmt> successors = successors(fromStmt);
        if (fromStmt instanceof JIfStmt) {
            // remove the first successor as if its a fallsthrough stmt and not a branch target
            return successors.skip(1);
        }
        return successors;
    }

    public boolean isStmtBranchTarget(@Nonnull Stmt targetStmt) {
        final List<Stmt> predecessors = predecessors(targetStmt).collect(Collectors.toList());
        ...
        if (iterator.hasNext()) {
            ...
            if (pred.branches()) {
                if (pred instanceof JIfStmt) {
                    return getBranchTargetsOf((JIfStmt) pred).anyMatch(s -> s == targetStmt);
                }
                return true;
            }
        }
    ...
    public Collection<Stmt> getLabeledStmts() {
        ...
        getNodes().filter(stmt -> stmt instanceof BranchingStmt).map(stmt -> (BranchingStmt) stmt).flatMap(this::getBranchTargetsOf).forEach(stmtList::add);
