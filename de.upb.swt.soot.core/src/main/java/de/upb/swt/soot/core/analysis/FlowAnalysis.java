...
public abstract class FlowAnalysis<A> extends AbstractFlowAnalysis<A> {
  ...
  enum Orderer {
    ...
    <F> List<Entry<Stmt, F>> newUniverse(
        StmtGraph stmtGraph, GraphView graphView, F entryFlow, boolean isForward) {
    ...
    private <F> Entry<Stmt, F>[] visitEntry(
        Map<Stmt, Entry<Stmt, F>> visited, Entry<Stmt, F> v, List<Stmt> out) {
      int n = out.size();
  ...
  final int doAnalysis(GraphView graphView, Map<Stmt, A> inFlow, Map<Stmt, A> outFlow) {
    ...
    final List<Entry<Stmt, A>> universe =
        Orderer.INSTANCE.newUniverse(graph, graphView, entryInitialFlow(), isForward());
