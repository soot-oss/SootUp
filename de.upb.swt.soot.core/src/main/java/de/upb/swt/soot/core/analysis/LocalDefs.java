...
public interface LocalDefs {
   ...
   * @param s a stmt that specifies the method context (location) to query for the definitions of
   *     the Local.
   * @return a list of stmts where the local is defined in the current method context. If there are
  ...
  public static final class Factory {
    ...
    /**
     * Creates a new LocalDefs analysis based on a given {@code StmtGraph}. If you don't trust the
     * input you should set {@code expectUndefined} to {@code true}.
     *
     * @see Body
     * @param body the body to work with
    ...
    public static LocalDefs newLocalDefs(Body body, boolean expectUndefined) {
      ...
      return new SimpleLocalDefs(
          body, expectUndefined ? FlowAnalysisMode.OmitSSA : FlowAnalysisMode.Automatic);
    }

    /**
     * Creates a new LocalDefs analysis based on a given {@code StmtGraph}. This analysis will be
  ...
  enum FlowAnalysisMode {
    ...
    Automatic,
    /**
     * Never use the SSA form, even if the stmt graph would allow for a flow-insensitive analysis
