...
public class StmtGraphBlockIterator implements Iterator<Stmt> {
  ...
  @Nonnull private final HashMap<Stmt, ArrayList<Stmt>> matchedInsertPointsSources = new HashMap<>();
  ...
  /**
   * Use DFS to detect loop edges, and store them in a HashMap
   * key: fromStmt, value: toStmt
   */
  ...
  /**build pred-adjacent-list for graph without loopEdges,
   * store it in a HashMap: key is Stmt, value a list of predecessors of the corresponding stmt
   * */
  ...
  /**
   * Each Trap's handlerStmt is a source node in StmtGraph. It handles, when the handlerBlock should be inserted in body.
   * The function is to calculate the stmt(InsertPoint). The handlerBlock should be inserted after this stmt directly.
   * @param trap
