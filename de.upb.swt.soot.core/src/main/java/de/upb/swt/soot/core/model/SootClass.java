...
/**
 * Soot's counterpart of the source languages class concept. Soot representation
 * of a Java class. They are usually created by a Scene, but can also be
...
public class SootClass extends AbstractClass<ClassSource> {
	...
	@Nonnull
	private final Supplier<Set<SootMethod>> _lazyMethods = Suppliers.memoize(this::lazyMethodInitializer);

	/**
	 * Gets the {@link Method methods} of this {@link SootClass} in an immutable
	 * set.
	 */
	@Nonnull
	...
	@Nonnull
	private final Supplier<Set<SootField>> _lazyFields = Suppliers.memoize(this::lazyFieldInitializer);
	...
	public Optional<SootField> getField(String name) {
		return this.getFields().stream().filter(field -> field.getSignature().getName().equals(name)).reduce((l, r) -> {
			throw new RuntimeException("ambiguous field: " + name);
		});
	}

	/**
	 * Returns the field of this class with the given sub-signature. If such a field
	 * does not exist, null is returned.
	 */
	@Nonnull
	public Optional<SootField> getField(@Nonnull FieldSubSignature subSignature) {
		return this.getFields().stream().filter(field -> field.getSubSignature().equals(subSignature)).findAny();
	}

	/**
	 * Attempts to retrieve the methodRef with the given signature, parameters and
	...
	public Optional<SootMethod> getMethod(@Nonnull MethodSignature signature) {
		return this.getMethods().stream().filter(method -> method.getSignature().equals(signature)).findAny();
	}
	...
	public Optional<SootMethod> getMethod(String name, Iterable<? extends Type> parameterTypes) {
		return this.getMethods().stream().filter(method -> method.getSignature().getName().equals(name)
				&& Iterables.elementsEqual(parameterTypes, method.getParameterTypes())).reduce((l, r) -> {
					throw new RuntimeException("ambiguous method: " + name);
				});
	}

	/**
	 * Attempts to retrieve the method with the given subSignature. This method may
	 * throw an AmbiguousMethodException if there are more than one method with the
	 * given subSignature. If no method with the given is found, null is returned.
	 */
	@Nonnull
	public Optional<SootMethod> getMethod(@Nonnull MethodSubSignature subSignature) {
		return this.getMethods().stream().filter(method -> method.getSubSignature().equals(subSignature)).findAny();
	}

	private final Supplier<Set<Modifier>> lazyModifiers = Suppliers.memoize(classSource::resolveModifiers);
	...
	private final Supplier<Set<ClassType>> lazyInterfaces = Suppliers.memoize(classSource::resolveInterfaces);
	...
	private final Supplier<Optional<ClassType>> lazySuperclass = Suppliers.memoize(classSource::resolveSuperclass);
	...
	private final Supplier<Optional<ClassType>> lazyOuterClass = Suppliers.memoize(classSource::resolveOuterClass);
	...
	/**
	 * Creates a new SootClass based on a new {@link OverridingClassSource}. This is
	 * useful to change selected parts of a {@link SootClass} without recreating a
	 * {@link ClassSource} completely. {@link OverridingClassSource} allows for
	 * replacing specific parts of a class, such as fields and methods.
	 */
	@Nonnull
	public SootClass withOverridingClassSource(Function<OverridingClassSource, OverridingClassSource> overrider) {
	...
	@Nonnull
	public SootClass withReplacedMethod(@Nonnull SootMethod toReplace, @Nonnull SootMethod replacement) {
		return new SootClass(new OverridingClassSource(classSource).withReplacedMethod(toReplace, replacement),
				sourceType);
	}
	...
	public SootClass withReplacedField(@Nonnull SootField toReplace, @Nonnull SootField replacement) {
		return new SootClass(new OverridingClassSource(classSource).withReplacedField(toReplace, replacement),
				sourceType);
	}
	...
	public SootClass withModifiers(@Nonnull Set<Modifier> modifiers) {
		return new SootClass(new OverridingClassSource(classSource).withModifiers(modifiers), sourceType);
	}

	@SuppressWarnings("OptionalUsedAsFieldOrParameterType")
	@Nonnull
	public SootClass withSuperclass(@Nonnull Optional<ClassType> superclass) {
		return new SootClass(new OverridingClassSource(classSource).withSuperclass(superclass), sourceType);
	}

	@SuppressWarnings("OptionalUsedAsFieldOrParameterType")
	@Nonnull
	public SootClass withOuterClass(@Nonnull Optional<ClassType> outerClass) {
		return new SootClass(new OverridingClassSource(classSource).withOuterClass(outerClass), sourceType);
	}
