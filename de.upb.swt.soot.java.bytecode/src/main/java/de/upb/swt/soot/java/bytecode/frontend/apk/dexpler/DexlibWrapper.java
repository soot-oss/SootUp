...
public class DexlibWrapper {
  ...
  private final soot.dexpler.DexClassLoader dexLoader = createDexClassLoader();
  ...
  public DexlibWrapper(File dexSource) {
    try {
      List<soot.dexpler.DexFileProvider.DexContainer<? extends DexFile>> containers = soot.dexpler.DexFileProvider.v().getDexFromSource(dexSource);
      this.dexFiles = new ArrayList<>(containers.size());
      for (soot.dexpler.DexFileProvider.DexContainer<? extends DexFile> container : containers) {
   ...
   * @return
   */
  protected soot.dexpler.DexClassLoader createDexClassLoader() {
    return new soot.dexpler.DexClassLoader();
  }
  ...
  public void initialize() {
    ...
    for (DexEntry<? extends DexFile> dexEntry : dexFiles) {
      ...
      for (ClassDef defItem : dexFile.getClasses()) {
        String forClassName = soot.dexpler.Util.dottedClassName(defItem.getType());
  ...
  public Dependencies makeSootClass(SootClass sc, String className) {
    if (soot.dexpler.Util.isByteCodeClassName(className)) {
      className = soot.dexpler.Util.dottedClassName(className);
    }
