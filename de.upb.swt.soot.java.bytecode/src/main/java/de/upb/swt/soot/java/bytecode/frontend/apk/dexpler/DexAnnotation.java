...
import de.upb.swt.soot.java.core.tag.annotation.AnnotationConstants;
import de.upb.swt.soot.java.core.tag.annotation.AnnotationDefaultTag;
import de.upb.swt.soot.java.core.tag.annotation.AnnotationTag;
import de.upb.swt.soot.java.core.tag.annotation.VisibilityAnnotationTag;
...
public class DexAnnotation {
                        ...
                        if (a.getType().equals("Ldalvik/annotation/AnnotationDefault;")) {
                            for (AnnotationElem ae : a.getElems()) {
                                if (ae instanceof AnnotationAnnotationElem) {
                                    ...
                                    for (JavaTaggedSootMethod sm : clazz.getMethods()) {
                                        String methodName = sm.getSootMethod().getName();
                                        if (defaults.containsKey(methodName)) {
                                            ...
                                            if (annotationType == null) {
                                                ...
                                                if (annotationType.equals(sm.getSootMethod().getReturnType())) {
                                                    isCorrectType = true;
                                                } else if (annotationType.equals(ARRAY_TYPE)) {
                                                    if (sm.getSootMethod().getReturnType() instanceof ArrayType) {
                                                        isCorrectType = true;
                                                    }
                                                }
                                            }

                                            if (isCorrectType && sm.getSootMethod().getParameterCount() == 0) {
                                                e.setName("default");
                                                ...
                                                // use the first one
                                                defaults.remove(sm.getSootMethod().getName());
                                            }
                                        }
                                    }
                                    for (Entry<String, AnnotationElem> leftOverEntry : defaults.entrySet()) {
                                        ...
                                        // return signature
                                        // does not match
                                        JavaTaggedSootMethod found = clazz.getMethodByNameUnsafe(leftOverEntry.getKey());
        ...
        switch (atypes) {
            ...
            case DALVIK_ANNOTATION_ENCLOSINGCLASS:
                ...
                for (AnnotationElement elem : a.getElements()) {
                    ...
                    // repair it
                    if (outerClass.equals(clazz.getSootClass().getName())) {
            ...
            case DALVIK_ANNOTATION_INNERCLASS:
                ...
                if (clazz.getSootClass().hasOuterClass()) {
                    // If we have already set an outer class from some other
                    // annotation, we use that
                    // one.
                    outerClass = clazz.getSootClass().getOuterClass().get().getClassName(); // TODO: KKwip
     ...
     *
     * @param vatg the visibility annotation tags for different visibility levels
