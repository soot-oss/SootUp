...
public class PrimitiveHierarchy implements IHierarchy {
  ...
  public static Collection<Type> getLeastCommonAncestor(Type a, Type b) {
    ...
    if (arePrimitives(a, b)) {
      if ((a instanceof PrimitiveType.ByteType && b instanceof PrimitiveType.Integer32767Type)
          || (b instanceof PrimitiveType.ByteType && a instanceof PrimitiveType.Integer32767Type)) {
        return Collections.singleton(PrimitiveType.getShort());
      } else if ((a instanceof PrimitiveType.CharType
              && (b instanceof PrimitiveType.ByteType || b instanceof PrimitiveType.ShortType))
          || (b instanceof PrimitiveType.CharType
              && (a instanceof PrimitiveType.ByteType || a instanceof PrimitiveType.ShortType))) {
   ...
   * child Todo[zw]: document of lattice.
   */
  public static boolean isAncestor(Type ancestor, Type child) {

    if (arePrimitives(ancestor, child)) {
      // The following ancestor-relationship checks are based on an type-ancestor-lattice: details
      // in the doc
      if (ancestor.equals(child)) {
        ...
        return child instanceof PrimitiveType.ByteType
            || child instanceof PrimitiveType.Integer32767Type
            || child instanceof PrimitiveType.Integer127Type
            || child instanceof PrimitiveType.Integer1Type
            || child instanceof BottomType;
        ...
        return (!(child instanceof PrimitiveType.BooleanType) && (child instanceof IntegerType))
            || child instanceof PrimitiveType.LongType
            || child instanceof BottomType;
      } else if (ancestor instanceof PrimitiveType.DoubleType) {
        return (!(child instanceof PrimitiveType.BooleanType) && (child instanceof IntegerType))
            || child instanceof PrimitiveType.LongType
            || child instanceof PrimitiveType.FloatType
            || child instanceof BottomType;
      } else {
        return false;
      }
    } else if (ancestor instanceof ArrayType && child instanceof ArrayType) {
      // If both types are ArrayType with IntegerType, we may have intermediate type of their bases.
      // Therefore, we may
      // need to check their ancestor-relationship for assign them final type.
      // This ancestor-relationship checks are stricter than above checks, e.g. because a local with
      // type int-array cannot be
