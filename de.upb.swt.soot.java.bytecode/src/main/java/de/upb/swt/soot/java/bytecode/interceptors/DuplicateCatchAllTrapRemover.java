...
public class DuplicateCatchAllTrapRemover implements BodyInterceptor {
    ...
    for (Trap trap1 : originalTraps){
      if(trap1.getExceptionType().getClassName().equals("java.lang.Throwable")){
        for (Trap trap2 : originalTraps){
          if(trap1 != trap2 && trap1.getBeginStmt() == trap2.getBeginStmt() && trap1.getEndStmt() == trap2.getEndStmt()
            ...
            for(Trap trap3 : traps){
              if(trap3 != trap1 && trap3 != trap2 && trap3.getExceptionType().getClassName().equals("java.lang.Throwable")){
                if(trapCoversStmt(originalBody, trap3, trap1.getHandlerStmt()) && trap3.getHandlerStmt() == trap2.getHandlerStmt()){
                  ...
                  traps.remove(trap2);
                  break;
                } else if (trapCoversStmt(originalBody, trap3, trap2.getHandlerStmt()) && trap3.getHandlerStmt() == trap1.getHandlerStmt()){
                  // c -> t2 -> t3 -> t1 && c -> t1
                  traps.remove(trap1);
                  break;
                }
  ...
  /**
   * Checks whether the given trap covers the given unit, i.e., there is an exceptional control flow from the given unit
   * to the given trap
   *
