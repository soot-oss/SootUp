...
public class AugHierarchy implements IHierarchy {
  ...
  public static Collection<Type> lcas_(Type a, Type b) {
    if (TypeResolver.typesEqual(a, b)) {
      ...
      if (PrimitiveType.isInstanceOfType(a, "boolean")
          || PrimitiveType.isInstanceOfType(b, "boolean")) {
        return Collections.emptyList();
      } else if ((PrimitiveType.isInstanceOfType(a, "byte")
              && PrimitiveType.isInstanceOfType(b, "short"))
          || (PrimitiveType.isInstanceOfType(b, "byte")
              && PrimitiveType.isInstanceOfType(a, "short"))) {
        return Collections.singletonList(PrimitiveType.getShort());
      } else if ((PrimitiveType.isInstanceOfType(a, "char")
              && (PrimitiveType.isInstanceOfType(b, "short")
                  || PrimitiveType.isInstanceOfType(b, "byte")))
          || (PrimitiveType.isInstanceOfType(b, "char")
              && (PrimitiveType.isInstanceOfType(a, "short")
                  || PrimitiveType.isInstanceOfType(a, "byte")))) {
        return Collections.singletonList(PrimitiveType.getInt());
      } else if (ancestor_(a, b)) {
        return Collections.singletonList(a);
      } else if (PrimitiveType.isInstanceOfType(a, "int")
          || PrimitiveType.isInstanceOfType(b, "int")) {
        return Collections.emptyList();
      } else {
        return Collections.singletonList(b);
      }
  ...
  public static boolean ancestor_(Type ancestor, Type child) {
    if (TypeResolver.typesEqual(ancestor, child)) {
      ...
      return child instanceof UnknownType || PrimitiveType.isInstanceOfType(child, "boolean");
    } else if (PrimitiveType.isInstanceOfType(ancestor, "byte")
        || PrimitiveType.isInstanceOfType(ancestor, "short")) {
      return child instanceof UnknownType
          || PrimitiveType.isInstanceOfType(child, "boolean")
      ...
      return child instanceof UnknownType
          || PrimitiveType.isInstanceOfType(child, "int")
          || PrimitiveType.isInstanceOfType(child, "byte")
          || PrimitiveType.isInstanceOfType(child, "char")
          || PrimitiveType.isInstanceOfType(child, "short");
