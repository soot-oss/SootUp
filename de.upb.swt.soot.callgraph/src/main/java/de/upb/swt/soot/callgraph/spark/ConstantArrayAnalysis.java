...
import de.upb.swt.soot.callgraph.spark.ConstantArrayAnalysis.ArrayState;
import de.upb.swt.soot.core.analyses.ForwardFlowAnalysis;
...
public class ConstantArrayAnalysis extends ForwardFlowAnalysis<ArrayState> {
  ...
  public ConstantArrayAnalysis(StmtGraph graph, Body b) {
    super(graph);
    ...
    for (Stmt u : b.getStmts()) {
      if (u instanceof AbstractDefinitionStmt) {
        Value rhs = ((AbstractDefinitionStmt) u).getRightOp();
        Type ty = rhs.getType();
        ...
        if (rhs instanceof JNewArrayExpr) {
          JNewArrayExpr nae = (JNewArrayExpr) rhs;
          if (nae.getSize() instanceof IntConstant) {
            int sz = ((IntConstant) nae.getSize()).getValue();
  ...
  @Override
  protected void flowThrough(ArrayState in, Stmt d, ArrayState out) {
    out.active.clear();
    out.active.or(in.active);
    out.state = Arrays.copyOf(in.state, in.state.length);
    if (d instanceof AbstractDefinitionStmt) {
      AbstractDefinitionStmt ds = (AbstractDefinitionStmt) d;
      ...
      if (rhs instanceof JNewArrayExpr) {
        ...
        Value naeSize = ((JNewArrayExpr) rhs).getSize();
        if (naeSize instanceof IntConstant) {
          int arraySize = ((IntConstant) naeSize).getValue();
        ...
        if (!(indexVal instanceof IntConstant)) {
          ...
          int index = ((IntConstant) indexVal).getValue();
  ...
  public ArrayTypes getArrayTypesBefore(Stmt s, Local arrayLocal) {
    ...
    for (int i = 0; i < toRet.possibleTypes.length; i++) {
      ...
      if (!ati.mustAssign.get(i)) {
        toRet.possibleTypes[i].add(NullType.getInstance());
      }
    }
    return toRet;
  }
