...
//    private final DexClassLoader dexLoader = createDexClassLoader();
    ...
    public DexLibWrapper(File dexSource) {
        try {
            ...
            List<DexFileProvider.DexContainer<? extends DexFile>> containers = new DexFileProvider().getDexFromSource(dexSource, 15);
...
//                    SootResolver.v().resolveClass(sootTypeName, SootClass.SIGNATURES);
    ...
    public ClassInformation getClassInformation(ClassType classType){
        String className = Util.isByteCodeClassName(classType.toString()) ? Util.dottedClassName(classType.toString()) : classType.toString();
