...
public class ApkAnalysisInputLocation<J extends SootClass<JavaSootClassSource>> implements AnalysisInputLocation<JavaSootClass> {
    ...
    private Map<String, EnumSet<ClassModifier>> extractDexFilesFromPath() {
        ...
        try {
            dexFromSource = new DexFileProvider().getDexFromSource(new File(apk_path.toString()), api_version);
        } catch (IOException e) {
    ...
    public String getAndroidJarPath(String jars, String apk) {
        ...
        if (!f.isFile()) {
            throw new RuntimeException(String.format("error: target android.jar %s does not exist.", jarPath));
        }
        return jarPath;
    }
    ...
    private int getAndroidAPIVersion(String jars, String apk) {
        ...
        while (api_version < maxAPI) {
            String jarPath = jars + File.separatorChar + "android-" + api_version + File.separatorChar + "android.jar";
