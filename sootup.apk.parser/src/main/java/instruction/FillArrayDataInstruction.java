...
public class FillArrayDataInstruction extends PseudoInstruction {
    ...
    public void computeDataOffsets(DexBody body) {
        ...
        for (int i = 0; i < numElements; i++) {
            JArrayRef jArrayRef = JavaJimple.getInstance().newArrayRef(arrayReference, IntConstant.getInstance(i));
            NumericConstant element = getArrayElement(elements.get(i), body, destRegister);
            if (element == null) {
                break;
            }
            JAssignStmt jAssignStmt = Jimple.newAssignStmt(arrayReference, element, StmtPositionInfo.createNoStmtPositionInfo());
            ...
            for (int register : usedRegisters) {
                if (i instanceof NewArrayInstruction) {
                    ...
                    if (instruction22c.getRegisterA() == register) {
                        ArrayType arrayType = (ArrayType) DexUtil.toSootType(((TypeReference) instruction22c.getReference()).getType(), 0);
