...
public class FilledNewArrayInstruction extends FilledArrayInstruction {
    ...
    public void jimplify(DexBody body) {
        ...
        JAssignStmt assign = Jimple.newAssignStmt(arrayLocal, arrayExpr, StmtPositionInfo.getNoStmtPositionInfo());
        body.add(assign);
        for (int i = 0; i < usedRegister; i++) {
            JArrayRef arrayRef = JavaJimple.getInstance().newArrayRef(arrayLocal, IntConstant.getInstance(i));
            JAssignStmt assign2 = Jimple.newAssignStmt(arrayRef, body.getRegisterLocal(regs[i]), StmtPositionInfo.getNoStmtPositionInfo());
            body.add(assign2);
        }
        setStmt(assign);
    }
