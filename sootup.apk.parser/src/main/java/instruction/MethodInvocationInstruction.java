...
public abstract class MethodInvocationInstruction extends DexLibAbstractInstruction implements DanglingInstruction{
    ...
    public void finalize(DexBody body, DexLibAbstractInstruction successor) {
        if(successor instanceof MoveResultInstruction){
            assignStmt = Jimple.newAssignStmt(body.getStoreResultLocal(), invocation, StmtPositionInfo.createNoStmtPositionInfo());
            setStmt(assignStmt);
            body.add(assignStmt);
            stmt = assignStmt;
        }
        else{
            JInvokeStmt jInvokeStmt = Jimple.newInvokeStmt(invocation, StmtPositionInfo.createNoStmtPositionInfo());
            setStmt(jInvokeStmt);
    ...
    protected void jimplifySpecial(DexBody body) {
        ...
        invocation = Jimple.newSpecialInvokeExpr(parameters.get(0),
                new MethodSignature(getClassTypeFromClassName(item.getDefiningClass()), new MethodSubSignature(item.getName(), convertParameterTypes(item.getParameterTypes()), DexUtil.toSootType(item.getReturnType(), 0))), buildArgs(parameters.subList(1, parameters.size())));
    ...
    protected void jimplifyStatic(DexBody body){
        ...
        invocation = Jimple.newStaticInvokeExpr(new MethodSignature(getClassTypeFromClassName(item.getDefiningClass()), item.getName(),convertParameterTypes(item.getParameterTypes()), DexUtil.toSootType(item.getReturnType(),0)));
        body.setDanglingInstruction(this);
    }
...
//            return;
//        }
        MethodSignature methodSignature = new MethodSignature(getClassTypeFromClassName(item.getDefiningClass()), item.getName(), convertParameterTypes(item.getParameterTypes()), DexUtil.toSootType(item.getReturnType(), 0));
        invocation = Jimple.newVirtualInvokeExpr(parameters.get(0), methodSignature, buildArgs(parameters.subList(1, parameters.size())));
        body.setDanglingInstruction(this);
    }
...
//        }
        MethodSignature methodSignature = new MethodSignature(getClassTypeFromClassName(item.getDefiningClass()), item.getName(), convertParameterTypes(item.getParameterTypes()), DexUtil.toSootType(item.getReturnType(), 0));
        invocation = Jimple.newInterfaceInvokeExpr(parameters.get(0), methodSignature, buildArgs(parameters.subList(1, parameters.size())));
        body.setDanglingInstruction(this);
    }
