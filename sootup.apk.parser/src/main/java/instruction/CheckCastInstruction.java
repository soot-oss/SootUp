...
public class CheckCastInstruction extends DexLibAbstractInstruction {
    ...
    public void jimplify(DexBody body) {
        ...
        Type checkCastType = DexUtil.toSootType(((TypeReference) checkCastInstr.getReference()).getType(), 0);

        JCastExpr castExpr = Jimple.newCastExpr(castValue, checkCastType);
        // generate "x = (Type) x"
        // splitter will take care of the rest
        JAssignStmt jAssignStmt = Jimple.newAssignStmt(castValue, castExpr, StmtPositionInfo.createNoStmtPositionInfo());
        setStmt(jAssignStmt);
