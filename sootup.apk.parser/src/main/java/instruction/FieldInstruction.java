...
public abstract class FieldInstruction extends DexLibAbstractInstruction {
    ...
    private JFieldRef getSootFieldRef(FieldReference fieldReference, boolean isStatic) {
        ...
        FieldSignature fieldSignature = new FieldSignature(Util.Util.getClassTypeFromClassName(className), fieldReference.getName(), DexUtil.toSootType(fieldReference.getType(), 0));
        if (isStatic) {
            JStaticFieldRef jStaticFieldRef = new JStaticFieldRef(fieldSignature);
            return jStaticFieldRef;
        } else {
            // TODO : Dont know which local to use here, as of now using null which will throw an error for sure.
    ...
    protected JAssignStmt getAssignStmt(DexBody body, Local sourceValue, ConcreteRef instanceField) {
        JAssignStmt assign;
        assign = Jimple.newAssignStmt(sourceValue, instanceField, StmtPositionInfo.createNoStmtPositionInfo());
        return assign;
    }
