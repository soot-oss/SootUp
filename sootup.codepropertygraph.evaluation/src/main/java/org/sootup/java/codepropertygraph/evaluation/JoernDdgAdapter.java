...
public class JoernDdgAdapter {
  private final JoernDdgGenerator joernDdgGenerator;
  private Map<Stmt, StmtPositionInfo> gotoStmtTargets = new HashMap<>();

  public JoernDdgAdapter(JoernDdgGenerator joernDdgGenerator) {
    this.joernDdgGenerator = joernDdgGenerator;
  }

  public PropertyGraph getDdg(Graph joernDdg) {
    if (joernDdg.vertices().size() == 0) {
      return new PropertyGraph();
    }

    gotoStmtTargets = new HashMap<>();

    // thisClassType = new ClassConstant(joernDdg.vertices().)
    // TypeDecl thisClass = method._typeDeclViaAstIn().get();
    // Method thisMethod =
    // (Method) joernDdg.vertices().filter(v -> v.label().equals("METHOD")).toIterator().next();
    ...
    for (Edge edge : joernDdgGenerator.getGraphEdges(joernDdg)) {
      ...
      // ddgGraph.addEdge(new PropertyGraphEdge(src, dst, "CFG"));
    }
