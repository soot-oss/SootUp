...
public class JoernCfgAdapter {
  ...
  private PropertyGraphNode getSootUpNode(StoredNode node, Graph astGraph) {
    ...
    if (node instanceof Block) {
      ...
      if (call.methodFullName().equals("<operator>.equals")) {
        String[] stmtTokens = call.code().split(" = ");
        String varName = stmtTokens[0];
        Stmt ifStmt = createIfStmt(astVars, positionInfo, stmtTokens[1], varName, "<operator>.equals");

        return new StmtPropertyGraphNode(
                ifStmt.toString(), NodeType.STMT, ifStmt.getPositionInfo());
      }

      if (call.methodFullName().equals("<operator>.notEquals")) {
        String[] stmtTokens = call.code().split(" != ");
        String varName = stmtTokens[0];
        Stmt ifStmt = createIfStmt(astVars, positionInfo, stmtTokens[1], varName, "<operator>.notEquals");

        return new StmtPropertyGraphNode(
                ifStmt.toString(), NodeType.STMT, ifStmt.getPositionInfo());
      }

      if (call.methodFullName().equals("<operator>.greaterThan")) {
        String[] stmtTokens = call.code().split(" > ");
        String varName = stmtTokens[0];
        Stmt ifStmt = createIfStmt(astVars, positionInfo, stmtTokens[1], varName, "<operator>.greaterThan");

        return new StmtPropertyGraphNode(
                ifStmt.toString(), NodeType.STMT, ifStmt.getPositionInfo());
      }

      if (call.methodFullName().equals("<operator>.lessThan")) {
        String[] stmtTokens = call.code().split(" < ");
        String varName = stmtTokens[0];
        Stmt ifStmt = createIfStmt(astVars, positionInfo, stmtTokens[1], varName, "<operator>.lessThan");
      ...
      if (call.methodFullName().equals("<operator>.greaterEqualsThan")) {
        String[] stmtTokens = call.code().split(" >= ");
        String varName = stmtTokens[0];
        Stmt ifStmt = createIfStmt(astVars, positionInfo, stmtTokens[1], varName, "<operator>.greaterEqualsThan");

        return new StmtPropertyGraphNode(
            ifStmt.toString(), NodeType.STMT, ifStmt.getPositionInfo());
      }

      if (call.methodFullName().equals("<operator>.lessEqualsThan")) {
        String[] stmtTokens = call.code().split(" <= ");
        String varName = stmtTokens[0];
        Stmt ifStmt = createIfStmt(astVars, positionInfo, stmtTokens[1], varName, "<operator>.lessEqualsThan");
  ...
  private Stmt createIfStmt(
      List<StoredNode> astVars,
      StmtPositionInfo positionInfo,
      String varValue,
      String varName,
    ...
    Value value = getConstantValue(varValue, varType);
    newLocal = new sootup.core.jimple.basic.Local(varName, value.getType());

    Stmt ifStmt;
    switch (expressionType) {
      case "<operator>.equals":
        ifStmt = new JIfStmt(new JEqExpr(newLocal, (Immediate) value), positionInfo);
        break;
      case "<operator>.notEquals":
        ifStmt = new JIfStmt(new JNeExpr(newLocal, (Immediate) value), positionInfo);
        break;
      case "<operator>.greaterThan":
        ifStmt = new JIfStmt(new JGtExpr(newLocal, (Immediate) value), positionInfo);
        break;
      case "<operator>.lessThan":
        ifStmt = new JIfStmt(new JLtExpr(newLocal, (Immediate) value), positionInfo);
        break;
      case "<operator>.greaterEqualsThan":
        ifStmt = new JIfStmt(new JGeExpr(newLocal, (Immediate) value), positionInfo);
        break;
      case "<operator>.lessEqualsThan":
        ifStmt = new JIfStmt(new JLeExpr(newLocal, (Immediate) value), positionInfo);
        break;
      default:
        ifStmt = null;
    }
