  ...
  private static void processFilePair(Path binFile, Path targetDir) {
    ...
    try {
      ...
      for (SootMethod sootupMethod : sootUpCfgGenerator.getMethods()) {
        // System.out.println(sootUpCfgGenerator.getMethodSignatureAsJoern(sootupMethod));
        // if (true) continue;
        ...
        try {
          ...
          comparer.compareCfg(joernCfg, sootUpCfg, methodSignatureAsJoern);
        } catch (RuntimeException e) {
          e.printStackTrace();
        }
      }

      // System.out.println("The number of methods is: " + sootUpCfgGenerator.getMethods().size());
