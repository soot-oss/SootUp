...
public class JoernCdgAdapter {
  private final JoernCdgGenerator joernCdgGenerator;
  private Map<Stmt, StmtPositionInfo> gotoStmtTargets = new HashMap<>();

  public JoernCdgAdapter(JoernCdgGenerator joernCdgGenerator) {
    this.joernCdgGenerator = joernCdgGenerator;
  }

  public PropertyGraph getCdg(Graph joernCdg) {
    if (joernCdg.vertices().size() == 0) {
      return new PropertyGraph();
    }

    gotoStmtTargets = new HashMap<>();

    // thisClassType = new ClassConstant(joernCdg.vertices().)
    // TypeDecl thisClass = method._typeDeclViaAstIn().get();
    // Method thisMethod =
    // (Method) joernCdg.vertices().filter(v -> v.label().equals("METHOD")).toIterator().next();
    ...
    for (Edge edge : joernCdgGenerator.getGraphEdges(joernCdg)) {
      ...
      // cdgGraph.addEdge(new PropertyGraphEdge(src, dst, "CFG"));
    }
