package org.sootup.java.codepropertygraph.evaluation.graph.comparison.converter.joern2sootup;

import io.shiftleft.codepropertygraph.generated.nodes.*;
import java.util.*;
import java.util.stream.Collectors;
import org.sootup.java.codepropertygraph.evaluation.MethodDetails;
import org.sootup.java.codepropertygraph.evaluation.MethodSignatureParser;
    ...
    if (call.dispatchType().equals("DYNAMIC_DISPATCH")) {
      ...
      MethodSubSignature methodSubSignature =
          new MethodSubSignature(
              ...
              methodDetails.getParameterTypes().stream()
                  .map(nodeTypeResolver::getNodeType)
                  .collect(Collectors.toList()),
              nodeTypeResolver.getNodeType(methodDetails.getReturnType()));
      ...
      MethodSignature methodSignature =
          new MethodSignature(
              new JavaClassType(
                  getClassName(methodDetails.getClassName()),
                  new PackageName(getPackageName(methodDetails.getClassName()))),
              methodSubSignature);
