...
public class Eval {
  ...
  private static final String RESULT_DIR =
      "sootup.codepropertygraph.evaluation/src/test/resources/result-logs";
  private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();

  public static void main(String[] args) throws IOException {
    try {
      createDirectoryIfNotExists(Paths.get(RESULT_DIR));
      deleteJsonFilesInDirectory(Paths.get(RESULT_DIR));
  ...
  private static void processFilePair(Path binFile, Path jarFile) {
    ...
    try {
      ...
      for (SootMethod sootupMethod : sootUpCfgGenerator.getMethods()) {
        try {
          if (sootupMethod.isAbstract() || sootupMethod.isNative()) continue; // Todo: handle abstract and native methods

          String methodSignatureAsJoern =
              sootUpCfgGenerator.getMethodSignatureAsJoern(sootupMethod);
          ...
          DotSerializer.Graph joernCfg = joernCfgGenerator.getCfg(joernMethod);
          PropertyGraph sootUpCfg = CfgCreator.convert(new MethodInfo(sootupMethod));

          DotSerializer.Graph joernAst = joernCfgGenerator.getAst(joernMethod);
