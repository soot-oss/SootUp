...
public class ClassHierarchyAnalysisPruningAlgorithmTest extends CallGraphTest {
  ...
  protected ClassHierarchyAnalysisAlgorithm createAlgorithm(JavaView view) {
    return new ClassHierarchyAnalysisAlgorithm(view, new PruningCallResolver(view));
  }
   ...
   * with a {@link PruningCallResolver}. It is expected that <code>methodB()</code> is not included
   * as a caller in the call graph, because the resolver excludes calls originating from <code>
   * methodB</code>.
   */
  @Test
  public void testFalsePruningCHA() {
    // CHA with a CallResolver excluding methodB() and all calls from methodB()
