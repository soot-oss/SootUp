          ...
          (val, allocSites) -> {
            ...
            if (!oldAllocSites.equals(allocSites)) {
              if (verbose) {
                ...
                //                        oldAllocSites.forEach(a ->
                // System.out.println(qilin.pta.getPag().valToAllocNode.get(a)));
                out.println("New Points-to set:" + allocSites.size());
                //                        allocSites.forEach(a ->
                // System.out.println(qilin.pta.getPag().valToAllocNode.get(a)));
              }
              int diff = allocSites.size() - oldAllocSites.size();
              if (diff > 0) count += diff;
              else if (diff < 0) count2 -= diff;
              if (verbose) {
