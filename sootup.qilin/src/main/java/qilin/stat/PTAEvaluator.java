 ...
 *
 * <p>Each new run of the PTA will over-write stats
 *
...
public class PTAEvaluator implements IEvaluator {
  ...
  public void begin() {
    ...
    exporter.collectMetric(
        "Used Memory Before:",
        (runtime.totalMemory() - runtime.freeMemory()) / GB + " GB"); // Print
    // used memory
    exporter.collectMetric(
        "Free Memory Before:", runtime.freeMemory() / GB + " GB"); // Print free memory
    exporter.collectMetric(
  ...
  public void end() {
    ...
    // from system
    exporter.collectMetric(
        "Used Memory After:", (runtime.totalMemory() - runtime.freeMemory()) / GB + " GB"); // Print
    // used memory
    exporter.collectMetric(
        "Free Memory After:", runtime.freeMemory() / GB + " GB"); // Print free memory
    exporter.collectMetric(
        "Total Memory After:", runtime.totalMemory() / GB + " GB"); // Print total available memory
    exporter.collectMetric(
        "Max Memory After:", runtime.maxMemory() / GB + " GB"); // Print Maximum available memory
