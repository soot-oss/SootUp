...
public class AsmMethodSource extends JSRInlinerAdapter implements BodySource {
    ...
    @Nonnull
    private final Map<Stmt, Stmt> replacedStmt = new HashMap<>();
    ...
    @Nonnull
    private final Map<LabelNode, JIdentityStmt> inlineExceptionHandlers = new HashMap<>();

    @Nonnull
    private final Map<LabelNode, Stmt> labelsToStmt = new HashMap<>();
...
//      setStmt(opr.insn, as);
            ...
            if (!danglingLabel.isEmpty()) {
                ...
                if (isLabelNode) {
                    ...
                    if (stmt instanceof JIdentityStmt) {
                        JIdentityStmt caughtExceptionRefStmt = (JIdentityStmt) stmt;
                        if (caughtExceptionRefStmt.getRightOp() instanceof JCaughtExceptionRef) {
    ...
    public Stream<Stmt> getStmtsThatUse(@Nonnull Value value) {
        Stream<Stmt> currentUses = insnToStmt.values().stream().filter(stmt -> stmt.getUses().contains(value));
