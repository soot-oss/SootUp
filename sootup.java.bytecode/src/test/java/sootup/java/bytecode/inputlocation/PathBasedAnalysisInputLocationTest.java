    ...
    public void testPackageName() {
        PathBasedAnalysisInputLocation pathBasedNamespace =
                new PathBasedAnalysisInputLocation.ClassFileBasedAnalysisInputLocation(
                        Paths.get("../shared-test-resources/ClassWithPackageName.class"),   // FIXME: to issue example
                        SourceType.Application);
        ArrayList<ClassType> sigs = new ArrayList<>();
        sigs.add(getIdentifierFactory().getClassType("ClassesPackageName.ClassWithPackageName"));

        JavaView javaView = new JavaView(pathBasedNamespace);
        assertTrue(javaView.getClasses().stream().allMatch( c -> c.getType().toString().equals("") && c.getType().toString().equals("") )); // FIXME

    }

    @Test
    public void testSingleClassWPackageName() {
        AnalysisInputLocation pathBasedNamespace = new JavaClassPathAnalysisInputLocation("../shared-test-resources/ClassWithPackageName.class", SourceType.Application);
        ArrayList<ClassType> sigs = new ArrayList<>();
        sigs.add(getIdentifierFactory().getClassType("ClassesPackageName.ClassWithPackageName"));
        testClassReceival(pathBasedNamespace, sigs, 1);
    }
