...
public class SootClass extends AbstractClass implements Serializable {
  ...
  public static class SootClassSurrogateBuilder implements DanglingStep, HierachyStep, SignatureStep, BodyStep, Build {
  ...
  @Nonnull public final static String INVOKEDYNAMIC_DUMMY_CLASS_NAME = "soot.dummy.InvokeDynamic";
  ...
  private <M extends SootClassMember> Set<M> initializeClassMembers(
    ...
    return
        iterableToStream(items)
            .peek(it -> it.setDeclaringClass(this))
...
//    }
  ...
  public void checkLevelIgnoreResolving(ResolvingLevel level) {
    ...
    if (currentLevel.getLevel() < level.getLevel()) {
      ...
      throw new RuntimeException("This operation requires resolving level " + level + " but " + classSignature.getClassName()
...
//    checkLevel(ResolvingLevel.SIGNATURES);
    ...
    return this.getFields().stream()
               ...
               .reduce((l, r) -> { throw new RuntimeException("ambiguous field: " + name); });
  }

  /**
   * Returns the field of this class with the given sub-signature. If such a field does not exist, null is returned.
   */
  @Nonnull
...
//    checkLevel(ResolvingLevel.SIGNATURES);
   ...
   */
  @Nonnull
  public Optional<SootMethod> getMethod(String name, Iterable<? extends TypeSignature> parameterTypes) {
//    checkLevel(ResolvingLevel.SIGNATURES);
    ...
    return
        this.getMethods().stream()
            ...
            .reduce((l, r) -> { throw new RuntimeException("ambiguous methodRef: " + name); });
  }
...
//    checkLevel(ResolvingLevel.HIERARCHY);
  ...
  public boolean isJavaLibraryClass() {
    return LibraryClassPatternHolder.LIBRARY_CLASS_PATTERN.matcher(classSignature.getClassName()).find();
  }
