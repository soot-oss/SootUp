...
public class AsmModuleClassSourceContent extends ClassNode implements IClassSourceContent {
  ...
  @Nonnull
  private SootModuleInfo.Build resolveHierarchy(@Nonnull IView view, @Nonnull JavaClassSignature cs)
    ...
    { // add exports
      for (ModuleExportNode exportNode : module.exports) {
        Iterable<JavaClassSignature> optionals = AsmUtil.asmIdToSignature(exportNode.modules);
...

    {
      ...
      for (ModuleOpenNode moduleOpenNode : module.opens) {
        Iterable<JavaClassSignature> optionals = AsmUtil.asmIdToSignature(moduleOpenNode.modules);
        ArrayList<JavaClassSignature> modules = new ArrayList<>();
        for (JavaClassSignature sootClassOptional : optionals) {
          if (sootClassOptional.isModuleInfo()) {
            modules.add(sootClassOptional);
  ...
  @Nonnull
  private SootModuleInfo.Build resolveSignature(@Nonnull IView view, @Nonnull JavaClassSignature cs)
      throws AsmFrontendException {
  ...
  @Nonnull
  private SootModuleInfo.Build resolveBody(@Nonnull IView view, @Nonnull JavaClassSignature cs)
      throws AsmFrontendException {
    Optional<AbstractClass> aClass = view.getClass(cs);
    if (!aClass.isPresent()) {
      throw new AsmFrontendException(String.format("Cannot resolve class %s", cs));
