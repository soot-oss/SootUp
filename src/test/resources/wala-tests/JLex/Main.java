...
class CSpec {
  ...
  /* Lexical States. */
  Hashtable m_states; /*
                       * Hashtable taking state indices (Integer) to state name (String).
                       */

  /* Regular Expression Macros. */
  Hashtable m_macros; /*
                       * Hashtable taking macro name (String) to corresponding char buffer that holds macro definition.
                       */
  ...
  Vector m_state_rules[]; /*
                           * An array of Vectors of Integers. The ith Vector represents the lexical state with index i. The
                           * contents of the ith Vector are the indices of the NFA start states that can be matched while in
  ...
  Hashtable m_dfa_sets; /*
                         * Hashtable taking set of NFA states to corresponding DFA state, if the latter exists.
                         */
...
class CEmit {
  ...
  /***************************************************************
   * Function: emit_imports Description: Emits import packages at top of generated source file.
   **************************************************************/
  /*
   * void emit_imports ( CSpec spec, OutputStream outstream ) throws java.io.IOException { set(spec,outstream);
   * 
   * if (CUtility.DEBUG) { CUtility.ASSERT(null != m_spec); CUtility.ASSERT(null != m_outstream); }
   */

  /*
   * m_outstream.println("import java.lang.String;"); m_outstream.println("import java.lang.System;");
   * m_outstream.println("import java.io.BufferedReader;"); m_outstream.println("import java.io.InputStream;");
   */
  /*
   * reset(); }
   */
  ...
  private void emit_construct() throws java.io.IOException {
    ...
    /*
     * if (m_spec.m_count_lines || true == m_spec.m_count_chars) { m_outstream.println("\t\tyy_buffer_prev_start = 0;"); }
     */
  ...
  private void emit_helpers() throws java.io.IOException {
     ...
     * m_outstream.println("\t\treturn yy_state_dtrans[state];"); m_outstream.println("\t}");
     */
     ...
     * m_outstream.println("\t\treturn yy_nxt[yy_rmap[current]][yy_cmap[lookahead]];"); m_outstream.println("\t}");
     */

    /* Function: yy_accept */
    /*
     * m_outstream.println("\tprivate int yy_accept (int current) {"); m_outstream.println("\t\treturn yy_acpt[current];");
     * m_outstream.println("\t}");
     */
  ...
  /***************************************************************
   * Function: emit_driver Description: Output an integer table as a string. Written by Raimondas Lencevicius 6/24/98;
   * reorganized by CSA 9-Aug-1999. From his original comments: yy_nxt[][] values are coded into a string by printing
   * integers and representing integer sequences as "value:length" pairs.
   **************************************************************/
  ...
  private void emit_driver() throws java.io.IOException {
    ...
    /*
     * m_outstream.println("\t\tint yy_state " + "= yy_initial_dtrans(yy_lexical_state);");
     */
    ...
    /*
     * m_outstream.println("\t\t\t\tyy_next_state = " + "yy_next(yy_state,yy_lookahead);");
     */
    ...
    if (NOT_EDBG) {
      ...
      m_outstream.println("\t\t\tjava.lang.System.out.println(\"Last accepting state = \"" + "+ yy_last_accept_state);");
      m_outstream.println("\t\t\tjava.lang.System.out.println(\"Next state = \"" + "+ yy_next_state);");
      m_outstream.println("\t\t\tjava.lang.System.out.println(\"Lookahead input = \"" + "+ ((char) yy_lookahead));");
    }
    ...
    /*
     * m_outstream.println("\t\t\t\t\tyy_error(YY_E_MATCH,false);"); m_outstream.println("\t\t\t\t\tyy_initial = true;");
     ...
     * m_outstream.println("\t\t\t\t\tyy_this_accept = yy_acpt[yy_state];");
     * m_outstream.println("\t\t\t\t\tif (YY_NOT_ACCEPT != yy_this_accept) {");
     * m_outstream.println("\t\t\t\t\t\tyy_last_accept_state = yy_state;"); m_outstream.println("\t\t\t\t\t}");
     */
    ...
    /*
     * m_outstream.println("\t\t\t\t\tyy_anchor " + "= yy_accept(yy_last_accept_state);");
     */
    ...
    /*
     * m_outstream.println("\t\t\t\t"); m_outstream.println("\t\t\t"); m_outstream.println("\t\t\t");
     * m_outstream.println("\t\t\t"); m_outstream.println("\t\t\t"); m_outstream.println("\t\t}");
     */
  }
...
class CMakeNfa {
  ...
  /***************************************************************
   * Function: allocate_BOL_EOF Description: Expands character class to include special BOL and EOF characters. Puts numeric
...
class CSimplifyNfa {
  ...
  /**
   * Compute minimum set of character classes needed to disambiguate edges. We optimistically assume that every character
   * belongs to a single character class, and then incrementally split classes as we see edges that require discrimination
   * between characters in the class. [CSA, 25-Jul-1999]
   */
...
class CMinimize {
  ...
  /***************************************************************
   * Function: fix_dtrans Description: Updates CDTrans table after minimization using groups, removing redundant transition
   * table states.
   **************************************************************/
        ...
        for (j = 1; j < group_size; ++j) {
          ...
          for (c = 0; c < m_spec.m_dtrans_ncols; ++c) {
            ...
            if (goto_first != goto_next
                && (goto_first == CDTrans.F || goto_next == CDTrans.F || m_ingroup[goto_next] != m_ingroup[goto_first])) {
              if (CUtility.DEBUG) {
...
class CNfa2Dfa {
  ...
  /* throws java.lang.CloneNotSupportedException */
  {
    ...
    for (istate = 0; nstates > istate; ++istate) {
      /*
       * CSA bugfix: if we skip all zero size rules, then an specification with no rules produces an illegal lexer (0 states)
       * instead of a lexer that rejects everything (1 nonaccepting state). [27-Jul-1999] if (0 ==
       * m_spec.m_state_rules[istate].size()) { m_spec.m_state_dtrans[istate] = CDTrans.F; continue; }
       */
    ...
    while (false == nfa_stack.empty()) {
      ...
      if (null != state.m_accept && state.m_label < bunch.m_accept_index) {
        ...
        if (CUtility.DEBUG) {
          ...
          CUtility.ASSERT(
              CSpec.NONE == bunch.m_anchor || 0 != (bunch.m_anchor & CSpec.END) || 0 != (bunch.m_anchor & CSpec.START));
        }
      }
  ...
  void move(Vector nfa_set, SparseBitSet nfa_bit, int b, CBunch bunch) {
    ...
    for (index = 0; index < size; ++index) {
      ...
      if (b == state.m_edge || (CNfa.CCL == state.m_edge && true == state.m_set.contains(b))) {
        ...
        bunch.m_nfa_set.addElement(state.m_next);
        /*
         * System.out.println("Size of bitset: " + bunch.m_nfa_bit.size()); System.out.println("Reference index: " +
  ...
  /***************************************************************
   * function: add_to_dstates Description: Takes as input a CBunch with details of a dfa state that needs to be created. 1)
   * Allocates a new dfa state and saves it in the appropriate CSpec vector. 2) Initializes the fields of the dfa state with
   * the information in the CBunch. 3) Returns index of new dfa.
   **************************************************************/
...
public class Main {
  ...
  public static void main(String arg[]) throws java.io.IOException {
    ...
    /*
     * Note: For debuging, it may be helpful to remove the try/catch block and permit the Exception to propagate to the top
     * level. This gives more information.
     */
    try {
...
class CInput {
  ...
  /***************************************************************
   * Function: getLine Description: Returns true on EOF, false otherwise. Guarantees not to return a blank line, or a line of
   * zero length.
   **************************************************************/
...
class CUtility {
  ...
  /*
   * static final boolean DEBUG = false; static final boolean SLOW_DEBUG = false; static final boolean DUMP_DEBUG = false;
   */
  ...
  /********************************************************
   * Function: bytencmp Description: Compares up to n elements of byte array a[] against byte array b[]. The first byte
   * comparison is made between a[a_first] and b[b_first]. Comparisons continue until the null terminating byte '\0' is
   * reached or until n bytes are compared. Return Value: Returns 0 if arrays are the same up to and including the null
   * terminating byte or up to and including the first n bytes, whichever comes first.
   *******************************************************/
  static int bytencmp(byte a[], int a_first, byte b[], int b_first, int n) {
    ...
    for (elem = 0; elem < n; ++elem) {
      /*
       * System.out.print((char) a[a_first + elem]); System.out.print((char) b[b_first + elem]);
       */
...
class CError {
  ...
  static final String errmsg[] = { "Malformed regular expression.", "Missing close parenthesis.",
      ...
      "Macro expansions nested too deeply.", "JLex has not been successfully initialized.", "Unexpected end-of-file found.",
      "Undefined or badly-formed JLex directive.", "Internal JLex error.", "Unitialized state name.",
      "Badly formed macro definition.", "Syntax error.", "Missing brace at start of lexical action.",
...
class CNfa {
   ...
   *******************************************************/
  int m_edge; /*
               * Label for edge type: character code, CCL (character class), [STATE, SCL (state class),] EMPTY, EPSILON.
               */
  ...
  CNfa m_next2; /*
                 * Another state with type == EPSILON and null if not used. The NFA construction should result in two
  ...
  /********************************************************
   * Constants: Edge Types Note: Edge transitions on one specific character are labelled with the character Ascii (Unicode)
   * codes. So none of the constants below should overlap with the natural character codes.
   *******************************************************/
...
class CLexGen {
  ...
  private Hashtable m_tokens; /*
                               * Hashtable that maps characters to their corresponding lexical code for the internal lexical
                               * analyzer.
                               */
  private CSpec m_spec; /*
                         * Spec class holds information about the generated lexer.
                         */
  private boolean m_init_flag; /*
                                * Flag set to true only upon successful initialization.
                                */

  private CMakeNfa m_makeNfa; /* NFA machine generator module. */
  private CNfa2Dfa m_nfa2dfa; /*
                               * NFA to DFA machine (transition table) conversion module.
                               */
  ...
  private CEmit m_emit; /*
                         * Output module that emits source code into the generated lexer file.
                         */
  ...
  private char[] getName() {
    ...
    /* Skip white space. */
    while (m_input.m_line_index < m_input.m_line_read && true == CUtility.isspace(m_input.m_line[m_input.m_line_index])) {
  ...
  private char[] packCode(char start_dir[], char end_dir[], char prev_code[], int prev_read, int specified)
    ...
    if (CUtility.DEBUG) {
      CUtility
          .ASSERT(INIT_CODE == specified || CLASS_CODE == specified || EOF_CODE == specified || EOF_VALUE_CODE == specified
              || INIT_THROW_CODE == specified || YYLEX_THROW_CODE == specified || EOF_THROW_CODE == specified);
    }
        ...
        switch (m_input.m_line[1]) {
          case '{':
            if (0 == CUtility.charncmp(m_input.m_line, 0, m_class_code_dir, 0, m_class_code_dir.length - 1)) {
              m_spec.m_class_code
                  = packCode(m_class_code_dir, m_class_code_end_dir, m_spec.m_class_code, m_spec.m_class_read, CLASS_CODE);
              break;
            }
          ...
          case 'e':
            if (0 == CUtility.charncmp(m_input.m_line, 0, m_eof_code_dir, 0, m_eof_code_dir.length - 1)) {
              m_spec.m_eof_code
                  = packCode(m_eof_code_dir, m_eof_code_end_dir, m_spec.m_eof_code, m_spec.m_eof_read, EOF_CODE);
              break;
            } else if (0 == CUtility.charncmp(m_input.m_line, 0, m_eof_value_code_dir, 0, m_eof_value_code_dir.length - 1)) {
              m_spec.m_eof_value_code = packCode(m_eof_value_code_dir, m_eof_value_code_end_dir, m_spec.m_eof_value_code,
                  m_spec.m_eof_value_read, EOF_VALUE_CODE);
              break;
            } else if (0 == CUtility.charncmp(m_input.m_line, 0, m_eof_throw_code_dir, 0, m_eof_throw_code_dir.length - 1)) {
              m_spec.m_eof_throw_code = packCode(m_eof_throw_code_dir, m_eof_throw_code_end_dir, m_spec.m_eof_throw_code,
          ...
          case 'i':
            if (0 == CUtility.charncmp(m_input.m_line, 0, m_integer_dir, 0, m_integer_dir.length - 1)) {
              ...
              break;
            } else if (0 == CUtility.charncmp(m_input.m_line, 0, m_init_code_dir, 0, m_init_code_dir.length - 1)) {
              m_spec.m_init_code
                  = packCode(m_init_code_dir, m_init_code_end_dir, m_spec.m_init_code, m_spec.m_init_read, INIT_CODE);
              break;
            } else if (0 == CUtility.charncmp(m_input.m_line, 0, m_init_throw_code_dir, 0,
                m_init_throw_code_dir.length - 1)) {
              m_spec.m_init_throw_code = packCode(m_init_throw_code_dir, m_init_throw_code_end_dir, m_spec.m_init_throw_code,
                  m_spec.m_init_throw_read, INIT_THROW_CODE);
              break;
            } else if (0 == CUtility.charncmp(m_input.m_line, 0, m_implements_dir, 0, m_implements_dir.length - 1)) {
              m_input.m_line_index = m_implements_dir.length;
              m_spec.m_implements_name = getName();
              break;
            } else if (0 == CUtility.charncmp(m_input.m_line, 0, m_ignorecase_dir, 0, m_ignorecase_dir.length - 1)) {
  ...
  /***************************************************************
   * Function: getStates Description: Parses the state area of a rule, from the beginning of a line. < state1, state2 ... >
   * regular_expression { action } Returns null on only EOF. Returns all_states, initialied properly to correspond to all
   * states, if no states are found. Special Notes: This function treats commas as optional and permits states to be spread
   * over multiple lines.
   **************************************************************/
  ...
  SparseBitSet getStates() throws java.io.IOException {
    ...
    if ('<' == m_input.m_line[m_input.m_line_index]) {
      ...
      while (true) {
        ...
        while (false == CUtility.isspace(m_input.m_line[m_input.m_line_index]) && ',' != m_input.m_line[m_input.m_line_index]
  ...
  private boolean expandMacro() {
    ...
    /*
     * Replace macro in new buffer, beginning by copying first part of line buffer.
     */
  ...
  private void saveMacro() {
    ...
    /*
     * Macro declarations are of the following form: macro_name macro_definition
     */
    ...
    in_ccl = false;
    saw_escape = false;
    while (false == CUtility.isspace(m_input.m_line[elem]) || true == in_quote || true == in_ccl || true == saw_escape) {
  ...
  /***************************************************************
   * Function: saveStates Description: Takes state declaration and makes entries for them in state hashtable in CSpec
   * structure. State declaration should be of the form: %state name0[, name1, name2 ...] (But commas are actually optional
   * as long as there is white space in between them.)
   **************************************************************/
  private void saveStates() {
    ...
    while (m_input.m_line_index < m_input.m_line_read) {
      ...
      m_spec.m_states.put(new String(m_input.m_line, start_state, count_state), new Integer(m_spec.m_states.size()));
  ...
  CAccept packAccept() throws java.io.IOException {
    ...
    while (true) {
      ...
      if ((insinglequotes || indoublequotes) && escaped)
        ...
        escaped = true;
      else if (!(insinglequotes || inslashcomment || instarcomment) && '\"' == m_input.m_line[m_input.m_line_index])
        indoublequotes = !indoublequotes; // unescaped double quote.
      else if (!(indoublequotes || inslashcomment || instarcomment) && '\'' == m_input.m_line[m_input.m_line_index])
  ...
  int advance() throws java.io.IOException {
    ...
    /*
     * if (m_input.m_line_index > m_input.m_line_read) { System.out.println("m_input.m_line_index = " +
     * m_input.m_line_index); System.out.println("m_input.m_line_read = " + m_input.m_line_read);
     * CUtility.ASSERT(m_input.m_line_index <= m_input.m_line_read); }
     */
    ...
    if (CUtility.FOODEBUG) {
      System.out
          .println("Lexeme: " + m_spec.m_lexeme + "\tToken: " + m_spec.m_current_token + "\tIndex: " + m_input.m_line_index);
    }
  ...
  private void details() {
    ...
    /*
     * if (null != m_spec.m_accept_vector && null != m_spec.m_anchor_array) { System.out.println();
     * System.out.println("\t** Accept States and Anchor Vector **"); print_accept(); }
     */
  }
 ...
 *
 * Re-implementation of the standard java.util.BitSet to support sparse sets, which we need to efficiently support unicode
 * character classes.
 */
 ...
 * permission notice and warranty disclaimer appear in supporting documentation, and that the name of Elliot Joel Berk not be
 ...
 * merchantability and fitness. In no event shall Elliot Joel Berk be liable for any special, indirect or consequential
